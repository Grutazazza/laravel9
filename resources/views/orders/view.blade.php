            ...
            <div class="col-8">
                @if(session()->has('success'))
                    @if(session()->get('success'))
                        <div class="alert alert-success">Заказ успешно отменён!</div>
                    @else
                    ...
                    @forelse($orders as $key=> $item)
                        <div class="accordion-item">
                            <h2 class="accordion-header" id="panelsStayOpen-headingOne">
                                <button class="accordion-button" type="button" data-bs-toggle="collapse" data-bs-target="#order_{{$key}}" aria-expanded="true" aria-controls="panelsStayOpen-headingOne">
                                    <div class="p-1">Номер заказа: {{$item->id}}</div>
                                    @if($item->status =='Отклонён')
                                        <div class="bg-danger text-white ml-1 p-1 rounded-2">Статус: {{$item->status}}</div>
                                    @elseif($item->status =='Завершён')
                            ...
                            <div id="order_{{$key}}" class="accordion-collapse collapse show" aria-labelledby="panelsStayOpen-headingOne">
                                <div class="accordion-body">
                                    ...
                                    <ul class="list-group">
                                        ...
                                        <li class="list-group-item">Адресс заказа: {{$item->address}}</li>
                                    </ul>
                                    @if($item->status == 'Новый')
                                        <p class="small">Мы мжете отменить заказ, если данный заказ является новым!</p>
                                        <a href="{{route('order.cancel',['order'=>$item->id])}}" class="btn btn-danger ml-1 mb-1">Отменить заказ</a>
                                        @if(\Illuminate\Support\Facades\Auth::user()->role=='admin')
                                            <a href="{{route('completed',['order'=>$item->id])}}" class="btn btn-success ml-1 mb-1">Заказ завершён</a>
                                        @endif
                                    @endif
                                </div>
                            </div>
